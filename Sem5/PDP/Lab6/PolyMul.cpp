...
void numberProdKaratsubaAux(std::vector<int> *a, int startA, int stopA, std::vector<int> *b, int startB, int stopB,
    ...
    if (threadsS3 > 0) {
        thS3 = new std::thread(numberProdKaratsubaAux, a, startA, middleA, b, startB, middleB, &resS3, 0,
                               threadsS1 - 1);
    } else {
    ...
    if (threadsS2 > 0) {
        thS2 = new std::thread(numberProdKaratsubaAux, a1plus0, 0, a1plus0->size(), b1plus0, 0, b1plus0->size(),
                               &prodS2, 0, threadsS2 - 1);
    } else {
