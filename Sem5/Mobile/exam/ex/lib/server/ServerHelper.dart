...
class ServerHelper<T extends IObject> {
  ...
  Future<http.Response> insert(T t) async {
    ...
    try {
      ...
      response = await http.post(this.url + "/file",
  ...
  Future<int> borrow(String id, String name) async {
    ...
    body['student'] = name;
    try {
      response = await http.post(this.url + "/borrow",
          body: json.encode(body),
          headers: {
            'Content-type': 'Application/json'
          }).timeout(const Duration(seconds: 3));
  ...
  Future<List<T>> getAllFromLocation(String name) async {
    final response = await http
        .get(this.url + "/files/" + name)
        .timeout(const Duration(seconds: 3));
  ...
  Future<List<String>> getLocations() async {
    final response = await http.get(this.url + '/locations');
    print(response.statusCode);
    var iObjectMapList = await json.decode(response.body);
    print("Map List as I got: " + iObjectMapList.toString());
    return List.generate(iObjectMapList.length, (i) {
      return iObjectMapList[i];
    });
  }
  ...
  Future<List<T>> synchronize() async {
    ...
    List<T> serverObjects = await this.getAll();
    ...
    serverObjects = await this.getAll();
