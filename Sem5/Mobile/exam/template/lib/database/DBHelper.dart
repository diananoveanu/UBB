...
class DBHelper<T extends IObject> {
  ...
  Future<Database> get db async {
    ...
    String path = p + "/" + _table.toLowerCase() + ".db";
    print("DBPATH: " + path);
    _db = await open(path);
    return _db;
  }
  ...
  Future<int> delete(int id) async{
    var dbClient = await db;
    return await dbClient.delete(_table,
    where: 'id=?', whereArgs: [id]);
  }

  Future<int> update(int id, IObject obj) async {
    var dbClient = await db;
    return await dbClient.update(_table, obj.toJsonDB(),
    where: 'id=?', whereArgs: [id]);
  }
  ...
  Future<int> getCount() async {
    var dbClient = await db;
    List<Map<String, dynamic>> x = await dbClient.rawQuery('SELECT COUNT(*) from $_table');
