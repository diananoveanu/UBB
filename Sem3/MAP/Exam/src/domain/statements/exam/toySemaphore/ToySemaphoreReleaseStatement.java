...
public class ToySemaphoreReleaseStatement implements IStatement {

    public String var;
    private static Lock lock = new ReentrantLock();
    public ToySemaphoreReleaseStatement(String var){
        this.var = var;
    }
        ...
        if(state.getSymbolTable().containsKey(var)){
            ...
            if(foundIndex.getType().equals(new IntegerType())){
                if(state.getToySemaphoreTable().exists(((IntegerValue)foundIndex).getValue())){
                    ...
                    semaphoreTuple.getIntegerList().remove(state.getID());
                }else{
                    lock.unlock();
    ...
    public String toString(){
        return "releaseToySemaphore(" + var + ")";
    }
}
